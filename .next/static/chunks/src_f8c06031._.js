...
function DeckManagerSheet({ characterId }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "DeckManagerSheet.useEffect": ()=>{
            if (deckData?.cards) {
                const cardCounts = {};
                const deckWithIds = deckData.cards.map({
                    "DeckManagerSheet.useEffect.deckWithIds": (cardName)=>{
                ...
                }["DeckManagerSheet.useEffect.deckWithIds"]);
                setDeck(deckWithIds);
            }
        }
    }["DeckManagerSheet.useEffect"], [
    ...
    const collectionPool = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "DeckManagerSheet.useMemo[collectionPool]": ()=>{
            const deckCardCounts = {};
            deck.forEach({
                "DeckManagerSheet.useMemo[collectionPool]": (c)=>{
                    deckCardCounts[c.name] = (deckCardCounts[c.name] || 0) + 1;
                }
            }["DeckManagerSheet.useMemo[collectionPool]"]);
            const totalCardCounts = {};
            collectionState.forEach({
                "DeckManagerSheet.useMemo[collectionPool]": (c)=>{
                    // For simplicity, let's assume starter deck indicates base count, then add inventory
                    const starterInfo = characterClass?.starterDeck.find({
                        "DeckManagerSheet.useMemo[collectionPool]": (sc)=>sc.name === c.name
                    }["DeckManagerSheet.useMemo[collectionPool]"]);
                    const inventoryInfo = inventoryData?.filter({
                        "DeckManagerSheet.useMemo[collectionPool]": (i)=>i.name === c.name && i.type === 'card'
                    }["DeckManagerSheet.useMemo[collectionPool]"]);
                    totalCardCounts[c.name] = (starterInfo?.count || 0) + (inventoryInfo?.length || 0);
                    if (!totalCardCounts[c.name]) {
                        const deckCount = deckData?.cards?.filter({
                            "DeckManagerSheet.useMemo[collectionPool]": (cardName)=>cardName === c.name
                        }["DeckManagerSheet.useMemo[collectionPool]"]).length || 0;
                        if (deckCount > 0) totalCardCounts[c.name] = deckCount;
                    }
                }
            }["DeckManagerSheet.useMemo[collectionPool]"]);
            return collectionState.filter({
                "DeckManagerSheet.useMemo[collectionPool]": (card)=>{
    ...
    const deckCardsData = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "DeckManagerSheet.useMemo[deckCardsData]": ()=>deck.map({
                "DeckManagerSheet.useMemo[deckCardsData]": (deckCard)=>{
                    const cardData = collectionState.find({
                        "DeckManagerSheet.useMemo[deckCardsData].cardData": (c)=>c.name === deckCard.name
                    }["DeckManagerSheet.useMemo[deckCardsData].cardData"]);
                    return {
                        ...cardData,
                        uniqueId: deckCard.id
                    };
                }
            }["DeckManagerSheet.useMemo[deckCardsData]"]).filter({
                "DeckManagerSheet.useMemo[deckCardsData]": (c)=>c.name
...
_s1(DeckManagerSheet, "xNqb6Rnog7lu7UBg4C91rjNEkCg=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$firebase$2f$provider$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useFirebase"],
...
_s1(AdventureClient, "Ishp1T7ccQfSbcck1pmu7Gj+Tqw=", false, function() {
    return [
...
//# sourceMappingURL=src_f8c06031._.js.map